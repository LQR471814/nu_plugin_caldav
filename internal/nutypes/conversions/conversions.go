...
func type_14293658896741725053_FromNu(v nu.Value) map[string][]string {
	dict := v.Value.(nu.Record)
	out := make(map[string][]string, len(dict))
	for k, v := range dict {
		out[k] = type_11669970230249425419_FromNu(v)
	}
	return out
}
...
func type_15678305217965743770_FromNu(v nu.Value) []nutypes.PropValueReplica {
	if v.Value == nil {
		return nil
	}
	arr := v.Value.([]nu.Value)
	...
	for i, e := range arr {
		out[i] = type_2040221911794545828_FromNu(e)
	}
	return out
}
...
func type_8207267565485682006_FromNu(v nu.Value) map[string][]nutypes.PropValueReplica {
	dict := v.Value.(nu.Record)
	out := make(map[string][]nutypes.PropValueReplica, len(dict))
	for k, v := range dict {
		out[k] = type_15678305217965743770_FromNu(v)
	}
	return out
}
...
func type_2040221911794545828_FromNu(v nu.Value) nutypes.PropValueReplica {
	out := nutypes.PropValueReplica{}
	record := v.Value.(nu.Record)
	var val nu.Value
	val, _ = record["value"]
	out.Value = type_15613163272824911089_FromNu(val)
	val, _ = record["params"]
	out.Params = type_14293658896741725053_FromNu(val)
	return out
}
...
func type_4505817543918974569_FromNu(v nu.Value) nutypes.EventReplica {
	...
	out.DatetimeStamp = type_12480522309550428545_FromNu(val)
	...
	val, _ = record["other"]
	out.Other = type_8207267565485682006_FromNu(val)
	return out
}
...
func type_4505817543918974569_ToNu(v nutypes.EventReplica) nu.Value {
	return nu.Value{Value: nu.Record{
		...
		"datetime_stamp":             type_12480522309550428545_ToNu(v.DatetimeStamp),
		...
		"other":                      type_8207267565485682006_ToNu(v.Other),
	}}
}
...
func type_2040221911794545828_ToNu(v nutypes.PropValueReplica) nu.Value {
	return nu.Value{Value: nu.Record{
		"value":  type_15613163272824911089_ToNu(v.Value),
		"params": type_14293658896741725053_ToNu(v.Params),
	}}
}
...
func type_8207267565485682006_ToNu(v map[string][]nutypes.PropValueReplica) nu.Value {
	dict := make(nu.Record, len(v))
	for k, v := range v {
		dict[k] = type_15678305217965743770_ToNu(v)
	}
	return nu.Value{Value: dict}
}
...
func type_14293658896741725053_ToNu(v map[string][]string) nu.Value {
	dict := make(nu.Record, len(v))
	for k, v := range v {
		dict[k] = type_11669970230249425419_ToNu(v)
	}
	return nu.Value{Value: dict}
}
func type_15678305217965743770_ToNu(v []nutypes.PropValueReplica) nu.Value {
	list := make([]nu.Value, len(v))
	for i, e := range v {
		list[i] = type_2040221911794545828_ToNu(e)
	}
	return nu.Value{Value: list}
}
